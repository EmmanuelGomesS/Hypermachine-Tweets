...
// Performs a redirect to the home page or redirect key
function redirect_to_home(){
  var redirect_url;
  var query_url = parseQuery(window.location.search.slice(1)).redirect;
  var default_url = window._home_page || '/';
  
  if (query_url) {
    var sp = unescape(query_url).split("|");
    var key = sp[0], value = sp[1];
    if (key == "ad_campaign") {
      redirect_url = window._ad_campaign_path.replace("_slug_", value);
    } else {
